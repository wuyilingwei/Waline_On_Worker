...
test("recipe preserves JWT on an ordinary overwrite and regenerates it when required", async () => {
  const { deploy } = await import(resolve(root, "recipe/recipe.js"));

  const overwrite = context({ mode: "overwrite", inputs: { domain: "waline.example.com" } });
